...
export const DateWeekSelector = () => {
  const { selectedWeekStart, setSelectedWeekStart, startDate, endDate, canNavigatePrevious, canNavigateNext } = useDateSelector()
  ...
  const handleDateInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(selectedDate.getTime()) && selectedDate >= startDate && selectedDate <= endDate) {
  ...
  return (
    <div className="flex items-center gap-2 ml-auto">
      ...
      <div className="relative">
        <div className="flex items-center gap-2 px-4 py-2 rounded-lg border border-border bg-background hover:bg-accent/50 cursor-pointer transition-colors">
