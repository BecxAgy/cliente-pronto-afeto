...
const DateSelectorContext = createContext<DateSelectorContextType | undefined>(
  undefined
);
...
export const DateSelectorProvider = ({
  ...
  // Define o dia inicial baseado na URL ou hoje/startDate
  const getInitialDay = (): Date => {
    if (initialSelectedDate) {
      const dateFromUrl = new Date(initialSelectedDate);
      dateFromUrl.setHours(0, 0, 0, 0);
      // Valida se está no range
      if (dateFromUrl >= startDate && dateFromUrl <= endDate) {
        return dateFromUrl;
      }
    }
    // Fallback: hoje se estiver no range, senão startDate
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    return today >= startDate && today <= endDate ? today : new Date(startDate);
  };

  const initialDay = getInitialDay();

  const [selectedWeekStart, setSelectedWeekStartState] = useState<Date>(
    getStartOfWeek(new Date(initialDay))
  );
...
export const useDateSelector = (): DateSelectorContextType => {
  ...
  if (context === undefined) {
    throw new Error(
      'useDateSelector must be used within a DateSelectorProvider'
    );
  }
  return context;
};
